...
// Capacity-only strategy for decode when no per-step host lengths are exposed.
// It deliberately favors one split once batch parallelism is already ample.
inline int chooseNumSplitsCapacityStrategy(size_t num_heads, size_t num_seqs,
                                           size_t seqlen_k, int sm_count) {
    if (sm_count <= 0 || num_heads == 0 || num_seqs == 0 || seqlen_k <= 256) return 1;
    if (num_seqs >= 8) return 1;
    if (num_seqs <= 2) {
        if (seqlen_k >= 4096) return 8;
        if (seqlen_k >= 1024) return 4;
        return 2;
    }
    if (seqlen_k >= 2048) return 4;
    if (seqlen_k >= 1024) return 2;
    return 1;
}
...
template <typename Tindex, typename Tdata>
INFINIOP_CUDA_KERNEL flashAttentionDecodeHd128GqaSplitKv(
    ...
    // Four warps in one CTA cover the four Q heads sharing this KV head.
    op::paged_attention::cuda::flashAttentionDecodeSplitKvWarpKernel<Tindex, Tdata, 128, 4>(
...
infiniStatus_t launch_decode_hd128_impl(
    ...
    bool num_splits_explicit = false;
    bool num_splits_auto = false;
    bool num_splits_invalid = false;
    const char *strategy_env = std::getenv("INFINIOP_FLASH_SPLITKV_STRATEGY");
    const bool requested_capacity_strategy = strategy_env && std::strcmp(strategy_env, "capacity_v1") == 0;
    if (const char *env = std::getenv("INFINIOP_FLASH_NUM_SPLITS")) {
        num_splits_explicit = true;
        if (std::strcmp(env, "auto") == 0) {
            fixed_num_splits = false;
            num_splits_auto = true;
        } else {
            char *end = nullptr;
            const long parsed = std::strtol(env, &end, 10);
            if (end != env && *end == '\0' && parsed > 0) {
                num_splits = static_cast<int>(parsed);
                fixed_num_splits = true;
            } else {
                // Invalid values retain the production default and never let
                // an experimental strategy silently replace the user input.
                num_splits_invalid = true;
            }
        }
    }
    if (num_splits < 1) {
    ...
    // capacity_v1 is scoped to the validated GQA decode shape. Numeric split
    // overrides always win; outside this scope auto uses the original heuristic.
    const bool capacity_strategy_in_scope = requested_capacity_strategy && use_split_auto &&
        (!num_splits_explicit || num_splits_auto) && !num_splits_invalid &&
        use_cta && alibi_slopes == nullptr && num_heads == 32 && num_kv_heads == 8 &&
        (dtype == INFINI_DTYPE_F16 || dtype == INFINI_DTYPE_BF16);
    // A positive/invalid numeric value is an explicit request: keep split
    // enabled and never replace it with an auto decision. Invalid values use
    // the production default (four splits) as a safe fallback.
    if (use_split_auto && num_splits_explicit && !num_splits_auto) {
        use_split = true;
    }
    const bool use_capacity_strategy = capacity_strategy_in_scope;
    const char *capacity_strategy_reason = !requested_capacity_strategy ? "not_requested" :
        (num_splits_invalid ? "invalid_num_splits_default" :
         (use_capacity_strategy ? "capacity_v1" :
          (num_splits_explicit && !num_splits_auto ? "numeric_override" :
           (use_split_auto ? "out_of_scope_fallback" : "split_disabled"))));
    ...
    auto dump_dispatch = [&](const char *path) {
        ...
        Sig cur{
            ...
            static_cast<int>(use_capacity_strategy),
        };

        if (has_last && cur.path == last.path && cur.dtype == last.dtype && cur.heads == last.heads && cur.kv_heads == last.kv_heads && cur.seqs == last.seqs && cur.pbs == last.pbs && cur.max_blocks == last.max_blocks && cur.cta_tile == last.cta_tile && cur.cta_threads == last.cta_threads && cur.split == last.split && cur.split_auto == last.split_auto && cur.num_splits == last.num_splits && cur.fixed == last.fixed && cur.gqa_fused == last.gqa_fused && cur.capacity_strategy == last.capacity_strategy) {
            return;
        }
        last = cur;
        has_last = true;
        ...
        fprintf(stderr,
                ...
                "pbs=%zu max_blocks=%zu cta_tile=%d cta_threads=%d split=%d split_auto=%d num_splits=%d fixed=%d gqa_fused=%d capacity_strategy=%d strategy_reason=%s\n",
                ...
                static_cast<int>(use_gqa_fused), static_cast<int>(use_capacity_strategy), capacity_strategy_reason);
    };

    // Split-kv auto mode: decide whether to split based on a heuristic.
    if (use_split_auto && (!num_splits_explicit || num_splits_auto) && !num_splits_invalid) {
        ...
        num_splits = use_capacity_strategy
            ? chooseNumSplitsCapacityStrategy(num_heads, num_seqs, seqlen_k, sm_count)
            : chooseNumSplitsHeuristic(num_heads, num_seqs, seqlen_k, sm_count);
        if (const char *dbg = std::getenv("INFINIOP_FLASH_DEBUG_SPLITS")) {
            if (std::strcmp(dbg, "1") == 0 || std::strcmp(dbg, "true") == 0) {
                ...
                if (last_seqlen_k != seqlen_k) {
                    ...
                    fprintf(stderr,
                            "[INFINIOP][paged_attention] splitkv auto(mode): strategy=%s sm=%d heads=%zu seqs=%zu seqlen_k~%zu -> num_splits=%d\n",
                            use_capacity_strategy ? "capacity_v1" : "heuristic", sm_count, num_heads, num_seqs, seqlen_k, num_splits);
                }
            }
        }
    ...
    const bool strategy_forces_nonsplit_cta = capacity_strategy_in_scope && !use_split;
    ...
    if (use_gqa_fused && !strategy_forces_nonsplit_cta && use_cta && !use_split && alibi_slopes == nullptr && num_kv_heads > 0 && num_heads == num_kv_heads * 4) {
    ...
    if (use_split) {
        ...
        if (use_gqa_split) {
            ...
            if (dtype == INFINI_DTYPE_F16) {
                flashAttentionDecodeHd128GqaSplitKv<Tindex, half><<<grid_split, block_split, 0, stream>>>(
                    partial_acc, partial_m, partial_l, static_cast<const half *>(q),
                    static_cast<const half *>(k_cache), static_cast<const half *>(v_cache),
                    block_tables, cache_lens, alibi_slopes, num_kv_heads, scale,
                    max_num_blocks_per_seq, page_block_size, q_stride, k_batch_stride,
            ...
            if (dtype == INFINI_DTYPE_BF16) {
                flashAttentionDecodeHd128GqaSplitKv<Tindex, __nv_bfloat16><<<grid_split, block_split, 0, stream>>>(
                    partial_acc, partial_m, partial_l, static_cast<const __nv_bfloat16 *>(q),
                    static_cast<const __nv_bfloat16 *>(k_cache), static_cast<const __nv_bfloat16 *>(v_cache),
                    block_tables, cache_lens, alibi_slopes, num_kv_heads, scale,
                    max_num_blocks_per_seq, page_block_size, q_stride, k_batch_stride,
