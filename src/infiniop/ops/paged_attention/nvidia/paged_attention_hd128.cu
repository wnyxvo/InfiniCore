...
infiniStatus_t launch_decode_hd128_impl(
    ...
    const bool use_capacity_strategy = strategy_env && std::strcmp(strategy_env, "capacity_v1") == 0;
    ...
    auto dump_dispatch = [&](const char *path) {
        ...
        fprintf(stderr,
                ...
                "pbs=%zu max_blocks=%zu cta_tile=%d cta_threads=%d split=%d split_auto=%d num_splits=%d fixed=%d gqa_fused=%d capacity_strategy=%d\n",
                ...
                static_cast<int>(use_gqa_fused), static_cast<int>(use_capacity_strategy));
    };
