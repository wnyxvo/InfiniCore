...
#pragma unroll
    ...
    for (int tile = start; tile < end; tile += TILE_TOKENS) {
        ...
        __syncthreads();
        if (active) {
            // Tile-level online softmax: one m/l update and one historical
            // accumulator rescale per tile, while retaining per-token weights.
            float scores[TILE_TOKENS];
            float probs[TILE_TOKENS];
#pragma unroll
...
#pragma unroll
            ...
            float alpha_shared = 1.0f;
            if (lane == 0) {
                for (int j = 0; j < tn; ++j) tile_max = fmaxf(tile_max, scores[j]);
                const float m_new = fmaxf(m, tile_max);
                const float alpha = exp2f(m - m_new);
                alpha_shared = alpha;
                float tile_sum = 0.0f;
                for (int j = 0; j < tn; ++j) {
                    probs[j] = exp2f(scores[j] - m_new);
                    tile_sum += probs[j];
                }
                l = alpha * l + tile_sum;
                m = m_new;
            }
            // Broadcast alpha computed before m was updated.
            const float alpha = __shfl_sync(0xffffffff, alpha_shared, 0);
            // Reconstruct the tile-normalized probabilities from lane 0's
            // score array. This keeps only TILE_TOKENS scalar registers.
            for (int j = 0; j < tn; ++j) {
                const float p = __shfl_sync(0xffffffff, (lane == 0) ? probs[j] : 0.0f, 0);
                for (int i = 0; i < DPT; ++i) {
                    const int dim = lane * DPT + i;
                    acc[i] = (j == 0 ? acc[i] * alpha : acc[i]) + p * static_cast<float>(sh_v[j][dim]);
                }
            }
        }
        __syncthreads();
...
#pragma unroll
    ...
    // Only thread 0 updates the thread-private m[g]/l[g] above.
    // Publish every group's normalization from that owner before the CTA sync.
    if (tid == 0) {
#pragma unroll
        for (int g = 0; g < NGROUPS; ++g) {
            inv_l_shared[g] = 1.0f / (l[g] + 1e-6f);
        }
    }
    if constexpr (NUM_WARPS == 1) {
        __syncwarp();
